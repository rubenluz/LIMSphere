...
class LabChatPage extends StatefulWidget {
  ...
  static final Map<String, int> _bgCounts = {
    for (final c in _channels) c.id: 0,
  };
  ...
  static void setActiveChannel(String? channelId) {
    ...
    SharedPreferences.getInstance().then(
      (p) => p.setInt('lab_chat_seen_$channelId', count),
    );
  }
...
class _LabChatPageState extends State<LabChatPage> {
    ...
    try {
      ...
      if (row == null && authUser.email != null) {
        ...
        if (row != null) {
          ...
          row = {...row, 'user_auth_uid': authUser.id};
        }
      }
  ...
  Future<void> _fetchMessages(String channelId) async {
    try {
      ...
      final all = rows.map(_messageFromRowWithUser).toList();
    ...
    try {
      ...
      final inserted = await _supabase
          ...
          .insert({
            ...
            if (replyTo != null) 'message_parent_id': replyTo.id,
      ...
      final msg = _messageFromRowWithUser({
        ...
        if (_currentUser != null) ...{
          ...
          'user_name':
              _currentUser!['user_name'] ?? _currentUser!['user_email'],
        },
      });
  ...
  Future<void> _softDelete(LabMessage msg) async {
    ...
    setState(() {
      ...
      if (msg.parentId != null) {
        final r = List<LabMessage>.from(_repliesByParent[msg.parentId!] ?? []);
  ...
  int get _pinnedCount => _messages.where((m) => m.pinned && !m.deleted).length;

  _Channel get _currentChannel => _channels.firstWhere((c) => c.id == _channel);
    ...
    if (msgs.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(
              _currentChannel.icon,
              size: 48,
              color: context.appTextMuted.withValues(alpha: 0.2),
            ),
            const SizedBox(height: 14),
            Text(
              'No messages yet in #${_currentChannel.label}',
            ...
            Text(
              ...
              style: _body(
                size: 12,
                color: context.appTextMuted.withValues(alpha: 0.6),
              ),
            ),
          ],
        ),
          ...
          Expanded(
            child: RichText(
              text: TextSpan(
                children: [
                  TextSpan(
                    text: 'Replying to ${msg.displayName}   ',
                    style: _body(
                      size: 11,
                      color: AppDS.accent,
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            if (institution != null)
              _infoRow(Icons.business_outlined, institution),
