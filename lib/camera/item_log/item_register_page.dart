    ...
    if (brand.isNotEmpty || ref.isNotEmpty) {
      try {
        ...
        _matches = rows.map<ReagentModel>((r) {
          ...
          final locName = loc is Map ? loc['location_name'] as String? : null;
    ...
    try {
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(child: _field(ctx, _brandCtrl, 'Brand')),
                        const SizedBox(width: 10),
                        Expanded(
                          child: _field(ctx, _refCtrl, 'Reference / Cat #'),
                        ),
                      ],
                    ),
                    const SizedBox(height: 10),
                    _dropdownField(
                      ...
                      items: ReagentModel.categoryOptions
                          ...
                          .toList(),
                      onChanged: (v) => setS(() => category = v ?? 'chemical'),
                    ),
                    const SizedBox(height: 10),
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 10),
                        Expanded(child: _field(ctx, unitCtrl, 'Unit')),
                      ],
                    ),
                    const SizedBox(height: 10),
                    _field(
      ...
      await Supabase.instance.client.from('reagents').insert({
        ...
        if (packSizeCtrl.text.isNotEmpty)
          'reagent_package_size': double.tryParse(packSizeCtrl.text.trim()),
        if (countCtrl.text.isNotEmpty)
          'reagent_container_count': int.tryParse(countCtrl.text.trim()),
        if (unitCtrl.text.isNotEmpty) 'reagent_unit': unitCtrl.text.trim(),
        if (lotCtrl.text.isNotEmpty) 'reagent_lot_number': lotCtrl.text.trim(),
      });
      unawaited(BackupService.instance.notifyCrudChange('reagents'));
      ...
      child: SizedBox(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Search by brand or catalog number',
              style: GoogleFonts.spaceGrotesk(
                color: AppDS.textMuted,
                fontSize: 13,
              ),
  ...
  Widget _buildMatchCard(ReagentModel r) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: context.appSurface,
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: AppDS.green.withValues(alpha: 0.35)),
      ),
      child: Row(
        children: [
          Expanded(
          ...
          FilledButton(
            ...
            style: FilledButton.styleFrom(
              backgroundColor: AppDS.accent.withValues(alpha: 0.15),
