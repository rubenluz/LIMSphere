...
class _FishStocksPageState extends State<FishStocksPage> {
  ...
  TextStyle get _tsNormal =>
      GoogleFonts.spaceGrotesk(fontSize: 12.5, color: context.appTextPrimary);
  TextStyle get _tsMono =>
      GoogleFonts.jetBrainsMono(fontSize: 12, color: context.appTextPrimary);
  TextStyle get _tsNormalMut =>
      GoogleFonts.spaceGrotesk(fontSize: 12.5, color: context.appTextMuted);
  TextStyle get _tsMonoMut =>
  ...
  String _normalizedTankId(Map<String, dynamic> row) {
    final raw = (row['fish_stocks_tank_id']?.toString() ?? '')
        .trim()
        .toUpperCase();
    final rack = (row['fish_stocks_rack']?.toString() ?? '')
        .trim()
        .toUpperCase();
    final dbRow = (row['fish_stocks_row']?.toString() ?? '')
        .trim()
        .toUpperCase();
  ...
  FishStock _stockFromRow(Map<String, dynamic> row) {
    ...
    return FishStock(
      ...
      line: (liveName?.isNotEmpty == true)
          ? liveName!
          : (line.isEmpty ? 'unknown' : line),
      ...
      volumeL: row['fish_stocks_volume_l'] != null
          ...
          : null,
      arrivalDate: arrivalRaw != null
          ? DateTime.tryParse(arrivalRaw.toString())
          : null,
      created: row['fish_stocks_created_at'] != null
          ? DateTime.tryParse(row['fish_stocks_created_at'].toString()) ??
                DateTime.now()
          : DateTime.now(),
    ...
    try {
      final rows =
          (await Supabase.instance.client
                      .from('fish_stocks')
                      .select(
                        '*, fish_lines!fish_stocks_line_id(fish_line_name, fish_line_date_birth)',
                      )
  ...
  Future<void> _loadActiveLines() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _activeLineNames = activeRows
              .map((r) => r['fish_line_name'] as String)
              .toList();
          _lineIdByName = {
  ...
  Future<void> _commitEdit(FishStock s, String key, String raw) async {
    ...
    switch (key) {
      case 'line':
        ...
        return;
      case 'males':
        s.males = int.tryParse(v) ?? s.males;
        dbCol = 'fish_stocks_males';
        dbVal = s.males;
        break;
      case 'females':
        s.females = int.tryParse(v) ?? s.females;
        dbCol = 'fish_stocks_females';
        dbVal = s.females;
        break;
      case 'juveniles':
        s.juveniles = int.tryParse(v) ?? s.juveniles;
        dbCol = 'fish_stocks_juveniles';
        dbVal = s.juveniles;
        break;
      case 'mortality':
        s.mortality = int.tryParse(v) ?? s.mortality;
        dbCol = 'fish_stocks_mortality';
        dbVal = s.mortality;
        break;
      case 'tankId':
        s.tankId = v.isEmpty ? s.tankId : v;
        dbCol = 'fish_stocks_tank_id';
        dbVal = s.tankId;
        break;
      case 'responsible':
        s.responsible = v;
        dbCol = 'fish_stocks_responsible';
        dbVal = v;
        break;
      case 'status':
        s.status = v;
        dbCol = 'fish_stocks_status';
        dbVal = v;
        break;
      case 'health':
        s.health = v;
        dbCol = 'fish_stocks_health_status';
        dbVal = v;
        break;
      case 'experiment':
        s.experiment = v.isEmpty ? null : v;
        dbCol = 'fish_stocks_experiment_id';
        dbVal = s.experiment;
        break;
      case 'cleaningInt':
  ...
  void _applyFilters() {
    ...
    if (_filterStatus != null)
      d = d.where((r) => r.status == _filterStatus).toList();
    if (_filterHealth != null)
      d = d.where((r) => r.health == _filterHealth).toList();
  ...
  Future<void> _exportCsv() async {
    ...
    buf.writeln(
      'Tank,Line,Status,Health,Males,Females,Juveniles,Total,Mortality,Responsible,Last Cleaning,Next Cleaning,Last Breeding,Notes',
    );
    for (final s in _filtered) {
      String esc(String? v) => '"${(v ?? '').replaceAll('"', '""')}"';
      final nextClean =
          (s.lastCleaning != null && s.cleaningIntervalDays != null)
          ? s.lastCleaning!
                .add(Duration(days: s.cleaningIntervalDays!))
                .toIso8601String()
                .substring(0, 10)
          : '';
      buf.writeln(
        [
          ...
          esc(s.responsible),
          s.lastCleaning != null
              ? s.lastCleaning!.toIso8601String().substring(0, 10)
              : '',
          nextClean,
          s.lastBreeding != null
              ? s.lastBreeding!.toIso8601String().substring(0, 10)
              : '',
          esc(s.notes),
        ].join(','),
      );
    ...
    try {
      ...
      final file = File(
        '${dir.path}/fish_stocks_${DateTime.now().millisecondsSinceEpoch}.csv',
      );
        ...
        children: [
          ...
          Row(
            children: [
              ...
              ...const ['active', 'empty', 'quarantine', 'retired'].map(
                (s) => Padding(
                  padding: const EdgeInsets.only(left: 6),
          ...
          Row(
            children: [
              ...
              _chip('All', _filterHealth == null, () {
                setState(() => _filterHealth = null);
                _applyFilters();
              }),
              ...const ['healthy', 'observation', 'treatment', 'sick'].map(
                (h) => Padding(
                  padding: const EdgeInsets.only(left: 6),
                  child: _chip(h, _filterHealth == h, () {
                    setState(() => _filterHealth = h);
                    _applyFilters();
                  }),
                ),
              ),
  ...
  Widget _cell(FishStock s, String key, double width, {bool mono = false}) {
    ...
    switch (key) {
      ...
      case 'line':
        ...
        break;
      case 'ageDays':
        val = s.ageDays > 0 ? '${s.ageDays}' : null;
        break;
      case 'ageMonths':
        val = s.ageMonths > 0 ? '${s.ageMonths}' : null;
        break;
      case 'males':
        val = '${s.males}';
        break;
      ...
      case 'mortality':
        ...
        break;
      case 'responsible':
        val = s.responsible.isEmpty ? null : s.responsible;
        break;
      case 'experiment':
        val = s.experiment;
        break;
      case 'feedingAmount':
        val = s.feedingAmount == null
            ? null
            : (s.feedingAmount! % 1 == 0
                  ? s.feedingAmount!.toInt().toString()
                  : s.feedingAmount!.toString());
        break;
      case 'cleaningInt':
        val = s.cleaningIntervalDays?.toString();
        break;
      default:
        val = null;
    }
    ...
    final readOnly =
        key == 'ageDays' || key == 'ageMonths' || key == 'nextCleaning';
    return GestureDetector(
        ...
        child: Padding(
          ...
          child: isEditing
              ? TextField(
                  ...
                  onTapOutside: (_) =>
                      _commitEdit(s, key, _editController.text),
                )
              : Text(
                  val ?? '—',
  ...
  Widget _dropdownCell(
    ...
    final isEditing =
        _editingCell?['id'] == s.id && _editingCell?['key'] == key;
    return GestureDetector(
      onDoubleTap: () {
        if (!context.canEditModule) {
          context.warnReadOnly();
