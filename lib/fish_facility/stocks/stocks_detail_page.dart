...
const _groups = <({String title, String icon, List<_Field> fields})>[
  ...
  (
    ...
    fields: [
      ...
      (
        key: 'fish_stocks_last_health_check',
  ...
  (
    ...
    fields: [
      (
        key: 'fish_stocks_temperature_c',
        label: 'Temperature (\u00b0C)',
        lines: 1,
      ),
      (key: 'fish_stocks_ph', label: 'pH', lines: 1),
      (
        key: 'fish_stocks_conductivity',
  ...
  (
    ...
    fields: [
      ...
      (
        key: 'fish_stocks_cleaning_interval_days',
  ...
  (
    title: 'Responsible',
    icon: 'responsible',
    fields: [(key: 'fish_stocks_responsible', label: 'Responsible', lines: 1)],
  ),
  (
    title: 'Notes',
    icon: 'notes',
    fields: [(key: 'fish_stocks_notes', label: 'Notes', lines: 5)],
  ),
];
...
class _TankDetailPageState extends State<TankDetailPage> {
  ...
  String? _loadedLineDateBirth;
  List<String> _loadedLineBreeders = [];
  ...
  int get _males =>
      int.tryParse(_ctrl['fish_stocks_males']?.text ?? '') ??
      (widget.tank.zebraMales ?? 0);
  int get _females =>
      int.tryParse(_ctrl['fish_stocks_females']?.text ?? '') ??
      (widget.tank.zebraFemales ?? 0);
  int get _juvs =>
      int.tryParse(_ctrl['fish_stocks_juveniles']?.text ?? '') ??
      (widget.tank.zebraJuveniles ?? 0);
  int get _total => _males + _females + _juvs;

  // ── Computed age from fish_line DOB ──────────────────────────────────────
  DateTime? get _dob =>
      _lineDateBirth != null ? DateTime.tryParse(_lineDateBirth!) : null;
  int get _ageDays =>
      _dob != null ? DateTime.now().difference(_dob!).inDays : -1;
  ...
  void initState() {
    super.initState();
    _recordTankId = widget.tank.zebraTankId;
    _applyTankSelection(_recordTankId);
  ...
  void _applyTankSelection(String tankId) {
    final parts = tankId.split('-');
    _selectedRack = parts.isNotEmpty && parts.first.isNotEmpty
        ? parts.first
        : 'R1';
    final rowPart = parts.length > 1 ? parts[1] : 'A1';
    _selectedRow = rowPart.isNotEmpty ? rowPart.substring(0, 1) : 'A';
    _selectedCol = rowPart.length > 1
        ? (int.tryParse(rowPart.substring(1)) ?? 1)
        : 1;
  }

  void _restoreLoadedViewState() {
    final loadedTankId = _data['fish_stocks_tank_id']?.toString();
    if (loadedTankId != null && loadedTankId.isNotEmpty) {
      _recordTankId = loadedTankId;
      _applyTankSelection(loadedTankId);
    } else {
      _applyTankSelection(_recordTankId);
    }
    final loadedLineName = _data['fish_stocks_line']?.toString().trim();
    _selectedLineId = _data['fish_stocks_line_id'] as int?;
    _selectedLineName = loadedLineName == null || loadedLineName.isEmpty
        ? null
        : loadedLineName;
    _lineDateBirth =
        _loadedLineDateBirth ?? _data['fish_stocks_dob']?.toString();
    _lineBreeders = List<String>.from(_loadedLineBreeders);
    _syncCtrls();
  }
  ...
  Future<void> _fetchLines() async {
    try {
      ...
      final idMap = {
        for (final r in rows)
          r['fish_line_name'] as String: r['fish_line_id'] as int,
      };
  ...
  Future<void> _fetchOccupiedPositions() async {
    try {
      final rows =
          await Supabase.instance.client
                  .from('fish_stocks')
                  .select('fish_stocks_tank_id')
              as List<dynamic>;
      if (!mounted) return;
      final ids = rows
          .map((r) => r['fish_stocks_tank_id'] as String)
          .where((id) => id != _recordTankId)
          .toSet();
      setState(() {
        _occupiedTankIds = ids;
  ...
  void _prefill() {
    ...
    _data = {
      ...
      'fish_stocks_sentinel_status': t.zebraTankType == 'sentinel'
          ? 'sentinel'
          : null,
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await Supabase.instance.client
          .from('fish_stocks')
          .select()
          .eq('fish_stocks_tank_id', _recordTankId)
          .maybeSingle();
      if (res != null) {
        _data = Map<String, dynamic>.from(res);
        _recordTankId =
            _data['fish_stocks_tank_id']?.toString() ?? _recordTankId;
        _applyTankSelection(_recordTankId);
        _syncCtrls();
        ...
        if (lineId != null) {
          lineRow = await Supabase.instance.client
              .from('fish_lines')
              .select(
                'fish_line_id, fish_line_name, fish_line_date_birth, fish_line_breeders',
              )
        ...
        if (lineRow != null) {
          ...
          _lineBreeders = breederStr.isEmpty
              ...
              : breederStr
                    ...
                    .where((s) => s.isNotEmpty)
                    .toList();
          _loadedLineDateBirth = _lineDateBirth;
          _loadedLineBreeders = List<String>.from(_lineBreeders);
        } else {
          _selectedLineId = lineId;
          _selectedLineName = lineName != null && lineName.isNotEmpty
              ? lineName
              : null;
          _lineDateBirth = _data['fish_stocks_dob']?.toString();
          _lineBreeders = [];
          _loadedLineDateBirth = _lineDateBirth;
          _loadedLineBreeders = [];
        }
    ...
    try {
      ...
      for (final g in _groups) {
        for (final f in g.fields) {
          ...
          if (f.key == '_line_breeders') {
            continue; // fish_line field, not saved here
          }
      ...
      final originalTankId = _recordTankId;
      if (_currentTankId != originalTankId) {
        // Moving to a different position: UPDATE by original ID
        await client
            .from('fish_stocks')
            .update(payload)
            .eq('fish_stocks_tank_id', originalTankId);
      } else {
        await client
            .from('fish_stocks')
            .upsert(payload, onConflict: 'fish_stocks_tank_id');
      }
      _recordTankId = _currentTankId;
      unawaited(BackupService.instance.notifyCrudChange('fish_stocks'));
      widget.onSaved?.call();
      await _fetchOccupiedPositions();
      await _load();
      ...
      builder: (_) => AlertDialog(
        ...
        content: RichText(
          ...
          text: TextSpan(
            ...
            children: [
              const TextSpan(
                text: 'This will permanently remove all data for\n',
              ),
              TextSpan(
                text: tankId,
  ...
  PreferredSizeWidget _buildMobileAppBar() => AppBar(
    ...
    title: Column(
      ...
      children: [
        ...
        if ((_selectedLineName ?? widget.tank.zebraLine) != null)
          Text(
            _selectedLineName ?? widget.tank.zebraLine!,
            style: TextStyle(
              fontSize: 11,
    ...
    actions: [
      ...
      PopupMenuButton<String>(
        ...
        itemBuilder: (_) => [
          ...
          if (_editMode && context.canEditModule)
            const PopupMenuItem(
              value: 'delete',
              child: ListTile(
                dense: true,
  ...
  PreferredSizeWidget _buildDesktopAppBar() {
    ...
    final status =
        _ctrl['fish_stocks_status']?.text ?? widget.tank.zebraStatus ?? '';
    return AppBar(
      backgroundColor: context.appSurface,
      foregroundColor: context.appTextPrimary,
      elevation: 0,
  ...
  Future<void> _openLinePage() async {
    ...
    try {
      ...
      Navigator.push(
        ...
        modulePageRoute(
          context: context,
          child: FishLineDetailPage(fishLine: line),
        ),
      );
    } catch (_) {}
  }
  ...
  Widget _buildLineLink() => InkWell(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
          bottom: BorderSide(color: _DS.accent.withValues(alpha: 0.3)),
        ),
      ),
      child: Row(
        children: [
            ...
            children: [
              if ((_ctrl['fish_stocks_temperature_c']?.text ?? '').isNotEmpty)
                Expanded(
                  child: _sidebarStat(
                    'TEMP',
                    '${_ctrl['fish_stocks_temperature_c']!.text}\u00b0C',
                    _DS.orange,
                  ),
                ),
              if ((_ctrl['fish_stocks_temperature_c']?.text ?? '').isNotEmpty &&
  ...
  Widget _buildIdentityLayout(List<_Field> fields) {
    final posField = fields
        .where((f) => f.key == 'fish_stocks_tank_id')
        .firstOrNull;
    final lineField = fields
        .where((f) => f.key == 'fish_stocks_line')
        .firstOrNull;
    final compactFields = fields
        .where(
          (f) => f.key != 'fish_stocks_tank_id' && f.key != 'fish_stocks_line',
        )
        .toList();
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (posField != null)
          SizedBox(width: 300, child: _buildField(posField)),
        if (posField != null) const SizedBox(width: 20),
        Expanded(
          child: LayoutBuilder(
            builder: (context, constraints) {
              final compactFieldWidth = constraints.maxWidth > 520
  ...
  Widget _buildField(_Field f) {
    final canEdit = context.canEditModule && _editMode;
    ...
    if (f.key == 'fish_stocks_tank_id') {
      return _editMode ? _buildTankIdField() : _buildReadOnlyTankIdField();
    }

    final rawVal = _data[f.key];
    final displayText =
        (f.key == 'fish_stocks_food_amount' &&
            rawVal is double &&
            rawVal % 1 == 0)
    ...
    if (!_editMode) {
      final displayValue = switch (f.key) {
        'fish_stocks_line' => _selectedLineName ?? ctrl.text,
        _ when _datePickers.contains(f.key) => _fmtDate(ctrl.text),
        _ => ctrl.text,
      };
      final statusColor = _statusColor(displayValue);
      return _buildReadOnlyField(
        label: f.label,
        value: displayValue,
        lines: f.lines,
        valueColor: f.key == 'fish_stocks_line'
            ? _DS.accent
            : (statusColor != AppDS.textMuted ? statusColor : null),
        trailing: f.key == 'fish_stocks_line' && _selectedLineName != null
            ? IconButton(
                icon: const Icon(
                ...
                tooltip: 'Open fish line',
                onPressed: _openLinePage,
              )
            : null,
      );
    }
    ...
    if (f.key == 'fish_stocks_line') {
      ...
      final currentVal =
          (_selectedLineName != null && _lines.contains(_selectedLineName))
          ? _selectedLineName
          : null;
      return InputDecorator(
        decoration: _dec(f.label),
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        color: _selectedRack == rack
                            ? _DS.accent.withValues(alpha: 0.1)
                            : context.appSurface,
                        borderRadius: BorderRadius.circular(6),
                        border: Border.all(
                          color: _selectedRack == rack
  ...
  Widget _buildReadOnlyTankIdField() => Container(
    padding: const EdgeInsets.all(12),
    decoration: BoxDecoration(
      color: context.appSurface3,
      borderRadius: BorderRadius.circular(8),
    ...
    child: Column(
      ...
      children: [
        ...
        Wrap(
          ...
          runSpacing: 8,
          children: [
            _readOnlyMetaChip('Rack', _selectedRack),
            _readOnlyMetaChip('Row', _selectedRow),
            _readOnlyMetaChip('Column', '$_selectedCol'),
            _readOnlyMetaChip(
              'Volume',
              _ctrl['fish_stocks_volume_l']?.text.isNotEmpty == true
                  ? '${_ctrl['fish_stocks_volume_l']!.text} L'
                  : widget.tank.volumeLabel,
            ),
          ],
        ),
