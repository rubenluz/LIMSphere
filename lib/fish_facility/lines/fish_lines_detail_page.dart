...
const _groups = <({String title, String icon, List<_Field> fields})>[
  ...
  (
    ...
    fields: [
      ...
      (
        key: 'fish_line_mutation_description',
...
String _fmtDateTime(String? s) {
  ...
  final timePart = parts[1].substring(
    0,
...
class _FishLineDetailPageState extends State<FishLineDetailPage> {
  ...
  void _prefill() {
    ...
    _data = {
      ...
      'fish_line_date_birth': l.fishlineDateBirth?.toIso8601String().split(
        'T',
      )[0],
      'fish_line_origin_lab': l.fishlineOriginLab,
      'fish_line_origin_person': l.fishlineOriginPerson,
      'fish_line_date_received': l.fishlineDateReceived
          ?.toIso8601String()
          .split('T')[0],
      ...
      'fish_line_cryo_date': l.fishlineCryoDate?.toIso8601String().split(
        'T',
      )[0],
  ...
  Future<void> _fetchAllLines() async {
    try {
      ...
      setState(
        () =>
            _allLines = rows.map((r) => r[FishSch.lineName] as String).toList(),
      );
    } catch (_) {}
  }
    ...
    try {
      final rows =
          await Supabase.instance.client
                  ...
                  .select(
                    '${FishSch.stockId}, ${FishSch.stockTankId}, ${FishSch.stockTankType}, '
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      payload[FishSch.lineBreeders] = _breeders.isEmpty
          ? null
          : _breeders.join(', ');
      ...
      builder: (_) => AlertDialog(
        ...
        content: RichText(
          ...
          text: TextSpan(
            ...
            children: [
              const TextSpan(
                text: 'This will permanently remove the record for\n',
              ),
              TextSpan(
                text: name,
                style: const TextStyle(
  ...
  Widget _buildMobile() => Scaffold(
    ...
    floatingActionButton: FloatingActionButton.extended(
      onPressed: _saving
          ? null
          : (context.canEditModule ? _save : context.warnReadOnly),
      backgroundColor: _DS.accent,
      foregroundColor: Colors.white,
      icon: _saving
          ? const SizedBox(
  ...
  PreferredSizeWidget _buildMobileAppBar() => AppBar(
    ...
    title: Column(
      ...
      children: [
        Text(
          widget.fishLine.fishlineName,
          style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15),
        ),
        if (widget.fishLine.fishlineAlias != null)
          Text(
            widget.fishLine.fishlineAlias!,
            style: TextStyle(
              fontSize: 11,
  ...
  PreferredSizeWidget _buildDesktopAppBar() {
    ...
    final status =
        _ctrl['fish_line_status']?.text ?? widget.fishLine.fishlineStatus ?? '';
    return AppBar(
      backgroundColor: context.appSurface,
      foregroundColor: context.appTextPrimary,
      elevation: 0,
      ...
      title: Column(
        ...
        children: [
          ...
          if (widget.fishLine.fishlineAlias != null ||
              widget.fishLine.fishlineZfinId != null)
            Text(
              [
                if (widget.fishLine.fishlineAlias != null)
                  widget.fishLine.fishlineAlias!,
                if (widget.fishLine.fishlineZfinId != null)
                  widget.fishLine.fishlineZfinId!,
              ].join('  ·  '),
              style: TextStyle(
                fontSize: 11,
                color: context.appTextMuted,
  ...
  Widget _buildField(_Field f) {
    final canEdit = context.canEditModule;
    if (_readOnlyKeys.contains(f.key)) {
      return _buildMetaField(f);
    }
    if (f.key == '_breeders') {
      return _buildBreedersField();
    }
    if (f.key == '_cryopreserved') {
      return _buildCryoToggle();
    }
      ...
      children: [
        ...
        Row(
          ...
          children: [
            Expanded(
              ...
              child: DropdownButtonFormField<String>(
                ...
                items: _allLines
                    .where(
                      (l) =>
                          !_breeders.contains(l) &&
                          l != widget.fishLine.fishlineName,
                    )
                    .map(
                      (l) => DropdownMenuItem(
                        value: l,
            ...
            FilledButton.icon(
              onPressed: canEdit
                  ? () => _addCustomBreeder(_breederCustomCtrl.text)
                  : null,
              style: FilledButton.styleFrom(
                backgroundColor: _DS.accent,
  ...
  Widget _buildMetaField(_Field f) {
    final rawKey = f.key == '_created_at'
        ? 'fish_line_created_at'
        : 'fish_line_updated_at';
              ...
              child: Row(
                children: [
                  ...
                  SizedBox(
                    ...
                    child: _stockNumCell(
                      males,
                      males > 0 ? _DS.accent : context.appTextMuted,
                    ),
                  ),
                  SizedBox(
                    width: 44,
                    child: _stockNumCell(
                      females,
                      females > 0 ? _DS.pink : context.appTextMuted,
                    ),
                  ),
                  SizedBox(
                    width: 44,
                    child: _stockNumCell(
                      juvs,
                      juvs > 0 ? _DS.orange : context.appTextMuted,
                    ),
                  ),
                  SizedBox(
                    width: 54,
                    child: _stockNumCell(
                      total,
                      total > 0 ? context.appTextPrimary : context.appTextMuted,
                    ),
                  ),
