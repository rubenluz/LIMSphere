...
class _AddStockDialogState extends State<AddStockDialog> {
  ...
  late final TextEditingController _foodOtherCtrl;
  late final TextEditingController _mixtureOtherACtrl;
  late final TextEditingController _mixtureOtherBCtrl;
  ...
  void initState() {
    ...
    final mixtureParts = fishFoodMixtureParts(p?.foodType);
    _foodOtherCtrl = TextEditingController(
      text: isFishFoodMixture(p?.foodType)
          ? ''
          : fishFoodCustomText(p?.foodType),
    );
    _mixtureOtherACtrl = TextEditingController(
      text: fishFoodCustomText(mixtureParts[0]),
    );
    _mixtureOtherBCtrl = TextEditingController(
      text: fishFoodCustomText(mixtureParts[1]),
    );
    _status = p?.status ?? 'active';
    _health = p?.health ?? 'healthy';
    _foodType = fishFoodTypeSelection(p?.foodType);
    _isFoodMixture = isFishFoodMixture(p?.foodType);
  ...
  void dispose() {
    ...
    _juvsCtrl.dispose();
    _foodOtherCtrl.dispose();
    _mixtureOtherACtrl.dispose();
    _mixtureOtherBCtrl.dispose();
    ...
    try {
      final rows =
          (await Supabase.instance.client
                      .from('fish_lines')
                      .select(
                        'fish_line_id, fish_line_name, fish_line_date_birth',
                      )
  ...
  Future<void> _submit() async {
    ...
    final foodType = normalizedFishFoodType(
      ...
      selected: _foodType,
      selectedCustom: _foodOtherCtrl.text,
      mixtureFirst: _mixtureFoodA,
      mixtureFirstCustom: _mixtureOtherACtrl.text,
      mixtureSecond: _mixtureFoodB,
      mixtureSecondCustom: _mixtureOtherBCtrl.text,
    );
    if ((_isFoodMixture || _foodType == fishFoodOtherOption) &&
        foodType == null) {
      setState(
        () => _error = _isFoodMixture
            ? 'Please select both foods for the mixture.'
            : 'Please enter the custom food.',
      );
      return;
    }
    setState(() {
    ...
    try {
      ...
      payload['fish_stocks_females'] =
          int.tryParse(_femalesCtrl.text.trim()) ?? 0;
      payload['fish_stocks_juveniles'] =
          int.tryParse(_juvsCtrl.text.trim()) ?? 0;
      if (foodType != null) payload['fish_stocks_food_type'] = foodType;
      if (_foodSource != null) payload['fish_stocks_food_source'] = _foodSource;
      if (_feedingSchedule != null) {
        payload['fish_stocks_feeding_schedule'] = _feedingSchedule;
      }
  ...
  Widget _foodTypeField() => Column(
    ...
    children: [
      CheckboxListTile(
        ...
        onChanged: (v) => setState(() => _isFoodMixture = v ?? false),
        dense: true,
          ...
          children: [
            Expanded(
              child: _mixtureFoodA == fishFoodOtherOption
                  ? _f('Custom food 1', _mixtureOtherACtrl)
                  : const SizedBox.shrink(),
            ),
            const SizedBox(width: 8),
            Expanded(
              child: _mixtureFoodB == fishFoodOtherOption
                  ? _f('Custom food 2', _mixtureOtherBCtrl)
                  : const SizedBox.shrink(),
            ),
          ],
        ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Red positions are already occupied.',
            style: GoogleFonts.jetBrainsMono(
              fontSize: 10,
              color: context.appTextMuted,
            ),
