...
extension _MobileView on _FishTanksPageState {
                    ...
                    child: Row(
                      children: [
                        Icon(
                          ...
                          size: 16,
                          color: context.canEditModule && _racks.length > 1
                              ? AppDS.red
                              : context.appTextMuted,
                        ),
                        const SizedBox(width: 8),
  ...
  Widget _buildMobileRack(double availW) {
    ...
    final innerW =
        availW - _mLabelW - 8; // 8 = SizedBox between label and cells
      ...
      switch (tank.zebraStatus) {
        ...
        case 'active':
          if (hasFish) {
            ...
            bg = _lightTankStandby.withValues(alpha: 0.12);
            border = _lightTankStandby.withValues(alpha: 0.40);
          }
          break;
        default:
          bg = (context.isDark ? AppDS.surface3 : _lightTankEmpty).withValues(
            alpha: context.isDark ? 0.80 : 0.96,
          );
          border = context.isDark
              ? const Color(0xFF334155)
              : _lightTankEmptyBorder;
      }
    }
