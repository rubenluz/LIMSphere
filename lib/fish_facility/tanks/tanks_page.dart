...
const _lightRackGreen = Color(0xFFF2FBF4);
const _lightTankStandby = Color(0xFF2E8B57);
const _lightTankEmpty = Color(0xFFFBFEFB);
const _lightTankEmptyBorder = Color(0xFFCFE3D2);
const _lightVolumeBadge = Color(0xFFEBF6ED);

// ─── Tank state helpers ───────────────────────────────────────────────────────
bool _isOccupied(ZebrafishTank t) =>
    t.zebraStatus != null &&
    t.zebraStatus != 'empty' &&
    t.zebraStatus != 'retired';
...
class _FishTanksPageState extends State<FishTanksPage> {
  ...
  void _applyStockRows(List<dynamic> rows) {
    ...
    for (final row in rows) {
      ...
      for (final list in _racks.values) {
        ...
        if (idx >= 0) {
          list[idx] = _fromRow(data, list[idx]);
          break;
        }
      }
    }
  ...
  Future<void> _loadCleaningTimeline() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _cleaningEvents = events;
          _cleaningLoading = false;
        });
      }
    } catch (_) {
  ...
  ZebrafishTank _fromRow(Map<String, dynamic> r, ZebrafishTank def) {
    ...
    return def.copyWith(
      ...
      zebraLastTankCleaning: r['fish_stocks_last_tank_cleaning'] != null
          ? DateTime.tryParse(r['fish_stocks_last_tank_cleaning'] as String)
          : null,
      zebraLastBreeding: r['fish_stocks_last_breeding'] != null
          ? DateTime.tryParse(r['fish_stocks_last_breeding'] as String)
          : null,
    );
  }
  ...
  Future<void> _persist(ZebrafishTank t) async {
    try {
      ...
      final persistedTankType = isSentinel
          ? 'holding'
          : (t.zebraTankType ?? 'holding');
      final payload = {
        ...
        'fish_stocks_last_tank_cleaning': t.zebraLastTankCleaning
            ?.toIso8601String()
            .substring(0, 10),
        'fish_stocks_last_breeding': t.zebraLastBreeding
            ?.toIso8601String()
            .substring(0, 10),
      };
      if (t.zebraId != null) {
  ...
  Future<void> _exportCsv() async {
    ...
    buf.writeln(
      'Rack,Tank ID,Type,Row,Column,Volume (L),Line,Males,Females,Juveniles,Responsible,Status,Health,Last Cleaning,Food Type,Feeding Schedule,Experiment,Notes',
    );
    for (final entry in allTanks) {
      ...
      buf.writeln(
        [
          ...
          t.zebraLastTankCleaning != null
              ? t.zebraLastTankCleaning!.toIso8601String().substring(0, 10)
              : '',
          esc(t.zebraFoodType),
    ...
    try {
      ...
      final file = File(
        '${dir.path}/tanks_${DateTime.now().millisecondsSinceEpoch}.csv',
      );
                      ...
                      children: [
                        ...
                        IntrinsicHeight(
                          child: Row(
                            ...
                            children: [
                              Expanded(
                                child: TanksWidgetActiveStocks(
                                  rackTanks: _rackTanks,
                                ),
                              ),
                              const SizedBox(width: 12),
                              Expanded(
                                child: TanksWidgetActiveFishLines(
                                  rackTanks: _rackTanks,
                                ),
                              ),
                              const SizedBox(width: 12),
                              Expanded(
                                child: TanksWidgetCleaningTimeline(
  ...
  Widget _buildRack(double availW) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: context.isDark ? AppDS.surface2 : _lightRackGreen,
  ...
  Widget _tankCell(ZebrafishTank tank, bool isTop) {
    ...
    if (isSent) {
      ...
      switch (tank.zebraStatus) {
        ...
        case 'active':
          if (hasFish) {
            bg = (dark ? AppDS.green : const Color(0xFF4D7C0F)).withValues(
              alpha: dark ? 0.45 : 0.22,
            );
            border = (dark ? AppDS.green : const Color(0xFF4D7C0F)).withValues(
              alpha: dark ? 0.80 : 0.55,
            );
          } else {
            bg = (dark ? AppDS.accent : _lightTankStandby).withValues(
              alpha: dark ? 0.22 : 0.12,
            );
            border = (dark ? AppDS.accent : _lightTankStandby).withValues(
              alpha: dark ? 0.55 : 0.40,
            );
          }
          break;
        default: // empty
          bg = (dark ? AppDS.surface3 : _lightTankEmpty).withValues(
            alpha: dark ? 0.80 : 0.96,
          );
          border = dark ? const Color(0xFF334155) : _lightTankEmptyBorder;
      }
    }
                      ...
                      if (!isTop && _showLabels && occupied) ...[
                        ...
                        if (hasFish)
                          ...
                          Text(
                            ...
                            style: GoogleFonts.spaceGrotesk(
                              fontSize: 9.0,
                              color: dark ? AppDS.accent : _lightTankStandby,
                          ...
                          Builder(
                            builder: (_) {
                              final days = DateTime.now()
                                  .difference(tank.zebraLastBreeding!)
                                  .inDays;
                              final Color c;
                              if (days >= 7) {
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (canEdit && (isOcc || (tank.zebraLine?.isNotEmpty ?? false)))
                _mi(
                  Icons.edit_outlined,
                  'Edit Tank',
                  context.appTextPrimary,
                ...
                () {
                  ...
                  showQuickPrintDialog(
                    ...
                    data: {
                      ...
                      'fish_stocks_females': (tank.zebraFemales ?? 0)
                          .toString(),
                      'fish_stocks_juveniles': (tank.zebraJuveniles ?? 0)
                          .toString(),
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              Text(
                label,
                style: GoogleFonts.spaceGrotesk(fontSize: 12.5, color: color),
              ),
            ],
          ),
        ),
  ...
  Set<String> get _occupiedTankIds => _racks.values
      .expand((l) => l)
      .where(_isOccupied)
      .map((t) => t.zebraTankId)
      .toSet();
  ...
  Widget _buildLegend() {
    return Wrap(
      ...
      children: [
        ...
        _li('Active, no fish', _lightTankStandby),
  ...
  Widget _li(String label, Color color, {bool dot = false}) => Row(
    mainAxisSize: MainAxisSize.min,
    children: [
      dot
          ? Container(
