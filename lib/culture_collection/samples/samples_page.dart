...
class _SamplesPageState extends State<SamplesPage> {
  ...
  List<SampleColDef> get _visibleCols {
    final ordered = _colOrder == null
        ...
        : [
            ..._colOrder!.map((k) {
              try {
                return sampleAllColumns.firstWhere((c) => c.key == k);
              } catch (_) {
                return null;
              }
  ...
  void _reorderCol(String colKey, int toVisibleIndex) {
    ...
    if (anchorKey == null) {
      ...
      mutable.insert(
        mutable.indexOf(anchorKey).clamp(0, mutable.length),
        colKey,
      );
    }
    setState(() {
  ...
  void _applySort() {
    ...
    _filtered.sort((a, b) {
      for (final key in _sortKeys) {
        ...
        if (isNum) {
          final ai =
              double.tryParse(a[key]?.toString() ?? '') ??
              (isAsc ? double.maxFinite : double.negativeInfinity);
          final bi =
              double.tryParse(b[key]?.toString() ?? '') ??
              (isAsc ? double.maxFinite : double.negativeInfinity);
          cmp = ai.compareTo(bi);
        } else {
  ...
  Future<void> _commitEdit(
    ...
    if (id == null) {
      _snack('Cannot save this sample because its database ID is missing.');
  ...
  Future<void> _openGpsPickerForRow(Map<String, dynamic> row) async {
    final sampleId = row['sample_id'];
    if (sampleId == null) {
      _snack('Cannot update this sample because its database ID is missing.');
      await _load();
      return;
    }

    final coordinates = parseSampleCoordinates(
      gps: row['sample_gps']?.toString(),
      latitude: row['sample_latitude']?.toString(),
      longitude: row['sample_longitude']?.toString(),
    );
    ...
    try {
      ...
      _snack('Could not save GPS position: $e');
    }
  }
  ...
  Future<void> _addRow() async {
    ...
    try {
      ...
      _applyFilter();
      debugPrint(
        '[SamplesPage] insert OK: sample_id=$newId '
  ...
  void _openDetail(Map<String, dynamic> row) {
    final sampleId = row['sample_id'];
    if (sampleId == null) {
      _snack('This stale sample has no database ID. Refreshing the list.');
      _load();
      return;
    }
    Navigator.push(
  ...
  void _selectAllRows() => setState(() {
    if (_selectedRowIds.length == _filtered.length) {
      ...
      _selectedRowIds.addAll(
        _filtered.map((r) => r['sample_id']),
      ); // Assuming 'sample_code' is the unique identifier
    }
  });
  ...
  Future<void> _exportSelectedCsv() async {
    ...
    final path =
        '${dir.path}/samples_export_${DateTime.now().millisecondsSinceEpoch}.csv';
    File(path)
  ...
  Future<void> _exportCsv() async {
    ...
    final dir = await getTemporaryDirectory();
    final path =
        '${dir.path}/samples_${DateTime.now().millisecondsSinceEpoch}.csv';
    File(path)
      ..createSync(recursive: true)
      ..writeAsStringSync(buf.toString());
    await OpenFilex.open(path);
      ...
      if (desktop) {
        return TextButton.icon(
          ...
          style: TextButton.styleFrom(
            foregroundColor: AppDS.toolbarIcon,
    ...
    return AppBar(
      ...
      title: Column(
        ...
        children: [
          ...
          Text(
            'Tap rows to select · tap column headers to pick columns',
            style: GoogleFonts.spaceGrotesk(
              fontSize: 10,
  ...
  Widget _buildFilterPanel() {
    List<String> uniqueVals(String key) =>
        _rows
    ...
    return Container(
      ...
      child: Column(
        ...
        mainAxisSize: MainAxisSize.min,
        children: [
          filterRow('Country', _filterCountry, uniqueVals('sample_country'), (
            v,
          ) {
          ...
          filterRow(
            'Habitat',
            _filterHabitatType,
            uniqueVals('sample_habitat_type'),
            (v) {
              setState(() => _filterHabitatType = v);
              _applyFilter();
            },
          ),
          const SizedBox(height: 8),
          filterRow('Project', _filterProject, uniqueVals('sample_project'), (
            v,
          ) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected ? AppDS.accent : context.appBorder2,
          ),
        ),
        child: Text(
          label,
  ...
  Widget _buildColumnManager() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(bottom: BorderSide(color: context.appBorder)),
        boxShadow: const [
          BoxShadow(color: AppDS.shadow, blurRadius: 4, offset: Offset(0, 2)),
        ],
      ),
      child: Column(
        mainAxisSize: MainAxisSize.min,
                            ...
                            children: [
                              ...
                              Expanded(
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (ctx, i) =>
                                      _buildDataRow(_filtered[i], i, cols),
                                ),
                              ),
                            ],
                          ),
            ...
            SizedBox(
              ...
              child: Center(
                child: Checkbox(
                  ...
                  checkColor: context.appSurface,
                  side: BorderSide(
                    color: context.appHeaderText.withValues(alpha: 0.38),
                    width: 1.5,
                  ),
                ),
              ),
                    ...
                    child: Text(
                      ...
                      style: AppDS.tableHeaderStyle.copyWith(
                        color: isColSelected
                            ? Colors.white
                            : col.readOnly
                            ? context.appHeaderText.withValues(alpha: 0.38)
  ...
  Widget _buildDataRow(
    Map<String, dynamic> row,
    int index,
    List<SampleColDef> cols,
  ) {
    final rowId = row['sample_id'];
    final isSelected = rowId != null && _selectedRowIds.contains(rowId);
    ...
    return GestureDetector(
      onTap: _selectionMode
          ? (rowId == null ? null : () => _toggleRowSelection(rowId))
          : null,
      child: Container(
  ...
  Widget _buildDataCell(
    ...
    final isEditing =
        _editingCell?['rowId'] == row['sample_id'] &&
        ...
        child: isEditing
            ...
            ? Row(
                children: [
                  ...
                  Tooltip(
                    message: context.canEditModule
                        ? 'Choose position on map'
                        : 'View position on map',
                    child: IconButton(
                      onPressed: _selectionMode
                          ? null
                          : () => _openGpsPickerForRow(row),
