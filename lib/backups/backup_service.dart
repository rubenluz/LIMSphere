...
class BackupService extends ChangeNotifier {
  ...
  Timer? _offlineRefreshDebounce;
  final Set<String> _offlineChangedTables = <String>{};
  ...
  Future<void> stop() async {
    ...
    _offlineRefreshDebounce = null;
    _offlineChangedTables.clear();
  ...
  Future<String> recommendedRootPath() async {
    ...
    return _joinPath(docs.path, 'LIMSphere');
  }
  ...
  Future<void> notifyCrudChange(String table) async {
    await loadSettings();
    if (!_settings.localMirrorEnabled) return;
    if (!_settings.offlineTables.contains(table)) return;
    _scheduleTableRefresh(table);
  }

  Future<String> refreshLocalMirror({
  ...
  void _scheduleTableRefresh(String table) {
    ...
    _offlineRefreshDebounce = Timer(delay, () {
      final changedTables = _offlineChangedTables.toList()..sort();
      _offlineChangedTables.clear();
      unawaited(refreshLocalMirror(reason: 'crud:${changedTables.join(',')}'));
    });
  }
