...
import 'package:blue_open_lims/models/lab_message.dart';
...
class _T {
  ...
  static const surface       = Color(0xFF0E1929);
  static const surface2      = Color(0xFF142034);
  static const surface3      = Color(0xFF1A2942);
  static const border        = Color(0xFF1E3252);
  static const border2       = Color(0xFF2A4474);
  static const accent        = Color(0xFF00C8F0);
  static const green         = Color(0xFF00D98A);
  static const red           = Color(0xFFFF4D6D);
  static const yellow        = Color(0xFFFFD60A);
  static const textPrimary   = Color(0xFFE2EBF9);
  static const textSecondary = Color(0xFF7A9CBF);
  static const textMuted     = Color(0xFF3D5A78);
  ...
  static TextStyle body({double size = 13, Color? color, FontWeight? weight}) =>
      GoogleFonts.dmSans(fontSize: size, color: color ?? textPrimary, fontWeight: weight);
}
...
class _LabChatPageState extends State<LabChatPage> {
  ...
  void _showSnack(String msg, {bool isError = false}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(msg, style: _T.body(size: 12)),
      backgroundColor: isError ? _T.red.withOpacity(0.9) : _T.surface2,
  ...
  Widget _buildChatHeader() {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Text(ch.label, style: _T.body(size: 15, weight: FontWeight.w700)),
        Container(
          margin: const EdgeInsets.symmetric(horizontal: 12),
          width: 1, height: 16, color: _T.border2),
        Text(
          '${_messages.where((m) => !m.deleted).length} messages',
          style: _T.mono(size: 10, color: _T.textMuted)),
        ...
        SizedBox(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Search…',
              hintStyle: _T.body(size: 12, color: _T.textMuted),
              prefixIcon: const Icon(Icons.search, size: 14, color: _T.textMuted),
              filled: true, fillColor: _T.surface2,
              border: _ob(), enabledBorder: _ob(),
              focusedBorder: _ob(color: _T.accent, w: 1.5),
        ...
        const Spacer(),
        if (_pinnedCount > 0) ...[
          _chip(Icons.push_pin, '$_pinnedCount pinned', _T.yellow,
  ...
  Widget _buildFilterBar() {
    return Container(
      ...
      color: _T.yellow.withOpacity(0.05),
      child: Row(children: [
        const Icon(Icons.filter_list, size: 12, color: _T.yellow),
        const SizedBox(width: 6),
        Text(
          _showPinnedOnly ? 'Showing pinned messages only'
              : 'Search: "$_search"',
          style: _T.body(size: 11, color: _T.yellow)),
        const Spacer(),
        GestureDetector(
          onTap: () => setState(() { _showPinnedOnly = false; _search = ''; }),
          child: const Icon(Icons.close, size: 13, color: _T.yellow)),
      ]),
    );
  }
      ...
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('Loading #${_currentChannel.label}…',
            style: _T.body(size: 12, color: _T.textMuted)),
        ]),
      );
    }
      ...
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(_currentChannel.icon, size: 38,
            color: _T.textMuted.withOpacity(0.25)),
          const SizedBox(height: 12),
          Text('No messages in #${_currentChannel.label}',
            style: _T.body(size: 13, color: _T.textMuted)),
        ]),
      );
    }
  ...
  Widget _buildReplyBanner() {
    ...
    return Container(
      ...
      child: Row(children: [
        const Icon(Icons.reply, size: 14, color: _T.accent),
        const SizedBox(width: 8),
        Expanded(
          child: RichText(text: TextSpan(children: [
            TextSpan(text: 'Replying to ${msg.displayName}   ',
              style: _T.body(size: 11, color: _T.accent, weight: FontWeight.w600)),
            TextSpan(
              text: msg.body.length > 70
                  ? '${msg.body.substring(0, 70)}…' : msg.body,
              style: _T.body(size: 11, color: _T.textSecondary)),
          ])),
        ),
        GestureDetector(
        ...
        child: Focus(
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: _ob(r: 10, color: _T.accent, w: 1.5),
      ...
      Tooltip(
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Icon(icon, size: 17, color: color ?? _T.textSecondary)),
        ),
      );
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        if (!_sidebarCollapsed) ...[
          const SizedBox(width: 8),
          const Icon(Icons.forum_outlined, size: 14, color: _T.accent),
          const SizedBox(width: 7),
          Text('Channels',
            style: _T.mono(size: 11, color: _T.accent, weight: FontWeight.w700)),
        ],
      ]),
    );
  }
          ...
          child: InkWell(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: isActive ? ch.color.withOpacity(0.12) : Colors.transparent,
                borderRadius: BorderRadius.circular(8),
                border: Border.all(
                  color: isActive ? ch.color.withOpacity(0.4) : Colors.transparent)),
              child: Row(children: [
                Icon(ch.icon, size: 15,
                  color: isActive ? ch.color : _T.textMuted),
                if (!_sidebarCollapsed) ...[
                  const SizedBox(width: 10),
                  Expanded(
                    child: Text(ch.label,
                  ...
                  if (count > 0)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: isActive ? ch.color.withOpacity(0.2) : _T.surface3,
                        borderRadius: BorderRadius.circular(10)),
                      child: Text('$count',
                        style: _T.mono(size: 9,
                          color: isActive ? ch.color : _T.textMuted)),
                    ),
                ],
              ]),
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          ...
          child: Center(
            child: Text('Y',
              style: _T.mono(size: 11, color: _T.green, weight: FontWeight.w700)),
          ),
        ),
        if (!_sidebarCollapsed) ...[
          const SizedBox(width: 10),
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('You', style: _T.body(size: 12, weight: FontWeight.w600)),
            Text('Online', style: _T.mono(size: 9, color: _T.green)),
          ]),
        ],
      ]),
    );
  ...
  Widget _chip(IconData icon, String label, Color color,
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: active ? color.withOpacity(0.15) : _T.surface2,
          borderRadius: BorderRadius.circular(6),
          border: Border.all(
            color: active ? color.withOpacity(0.4) : _T.border)),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(icon, size: 12, color: active ? color : _T.textSecondary),
  ...
  OutlineInputBorder _ob({
    double r = 8, Color color = _T.border, double w = 1.0}) =>
      OutlineInputBorder(
...
class _MessageBubbleState extends State<_MessageBubble> {
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        color: _hovered
            ? _T.surface2.withOpacity(0.45)
            : msg.pinned ? _T.yellow.withOpacity(0.025) : Colors.transparent,
        child: Padding(
            ...
            children: [
              ...
              if (msg.pinned)
                Padding(
                  ...
                  child: Row(children: [
                    const Icon(Icons.push_pin, size: 10, color: _T.yellow),
                    const SizedBox(width: 4),
                    Text('Pinned', style: _T.mono(size: 9, color: _T.yellow)),
                  ]),
                ),
                      ...
                      ? (_hovered
                          ? Center(
                              child: Text(_formatTime(msg.createdAt),
                                style: _T.mono(size: 8, color: _T.textMuted)))
                          : const SizedBox())
                      : Container(
                          width: 32, height: 32,
                          decoration: BoxDecoration(
                          ...
                          child: Center(
                            child: Text(
                              ...
                              style: _T.mono(size: 12, color: aColor,
                        ...
                        Padding(
                          ...
                          child: Row(children: [
                            ...
                            Text(_formatDate(msg.createdAt),
                              style: _T.mono(size: 9.5, color: _T.textMuted)),
                            if (msg.edited) ...[
                              const SizedBox(width: 6),
                              Text('edited',
                                style: _T.mono(size: 9, color: _T.textMuted)),
                            ],
                            const Spacer(),
                            AnimatedOpacity(
  ...
  Widget _buildBodyText(String text) {
    ...
    if (!tagRx.hasMatch(text)) {
      return Text(text, style: _T.body(size: 13.5));
    }
    ...
    for (final m in tagRx.allMatches(text)) {
      if (m.start > last) {
        spans.add(TextSpan(
          text: text.substring(last, m.start), style: _T.body(size: 13.5)));
      }
    ...
    if (last < text.length) {
      spans.add(TextSpan(
        text: text.substring(last), style: _T.body(size: 13.5)));
    }
  ...
  Widget _buildEditField(LabMessage msg) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      TextField(
        ...
        decoration: InputDecoration(
          filled: true, fillColor: _T.surface3,
          border: _eb(), enabledBorder: _eb(),
          focusedBorder: _eb(color: _T.accent),
      ...
      Row(children: [
        _pill('Save',   _T.accent, () => widget.onSaveEdit(msg)),
        const SizedBox(width: 8),
        _pill('Cancel', _T.textMuted, widget.onCancelEdit),
        const SizedBox(width: 10),
        Text('Enter to save', style: _T.mono(size: 9, color: _T.textMuted)),
      ]),
    ]);
  }

  OutlineInputBorder _eb({Color color = _T.border}) =>
      OutlineInputBorder(
  ...
  Widget _pill(String label, Color color, VoidCallback onTap) =>
      InkWell(
        ...
        child: Container(
          ...
          child: Text(label,
            style: _T.body(size: 11, color: color, weight: FontWeight.w700)),
        ),
      );
    ...
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        _aBtn(
          ...
          () => widget.onPin(msg),
          color: msg.pinned ? _T.yellow : null),
      ...
      Tooltip(
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Icon(icon, size: 14, color: color ?? _T.textSecondary)),
        ),
      );
  ...
  Widget _buildReplies() {
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
        border: Border(left: BorderSide(color: _T.border2, width: 2))),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
          ...
          return Padding(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                width: 24, height: 24,
                decoration: BoxDecoration(
                  color: aColor.withOpacity(0.15), shape: BoxShape.circle),
                child: Center(
                  child: Text(
                    r.displayName.isNotEmpty
                        ? r.displayName[0].toUpperCase() : '?',
                    style: _T.mono(size: 9, color: aColor,
                      weight: FontWeight.w700)),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Text(r.displayName,
                      style: _T.body(size: 11.5, color: aColor,
                        weight: FontWeight.w700)),
                    const SizedBox(width: 6),
                    Text(_formatDate(r.createdAt),
                      style: _T.mono(size: 8.5, color: _T.textMuted)),
                    if (r.edited) ...[
                      const SizedBox(width: 4),
                      Text('edited',
                        style: _T.mono(size: 8, color: _T.textMuted)),
                  ...
                  Text(r.body, style: _T.body(size: 12.5)),
                ]),
              ),
            ]),
          );
...
class _ContextPickerDialogState extends State<_ContextPickerDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: Text('Attach Context Reference',
        style: _T.body(size: 15, weight: FontWeight.w700)),
      content: SizedBox(
        width: 380,
        child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            children: _types.map((t) {
              ...
              return InkWell(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: sel ? t.$3.withOpacity(0.15) : _T.surface3,
                    borderRadius: BorderRadius.circular(6),
                    border: Border.all(
                      color: sel ? t.$3.withOpacity(0.5) : _T.border)),
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(t.$2, size: 13, color: sel ? t.$3 : _T.textMuted),
                    const SizedBox(width: 5),
                    Text(t.$1,
                      style: _T.body(size: 12,
                        color: sel ? t.$3 : _T.textSecondary,
      ...
      actions: [
        TextButton(onPressed: () => Navigator.pop(context),
          child: Text('Cancel', style: _T.body(size: 13, color: _T.textMuted))),
        ElevatedButton(
  ...
  Widget _f(String label, TextEditingController ctrl,
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label,
          style: _T.body(size: 11, color: _T.textMuted, weight: FontWeight.w700)),
