...
class _ExcelImportPageState extends State<ExcelImportPage> {
  ...
  Map<String, dynamic> _autoSampleFromStrain(
      Map<String, String> s, int number) {
    return {
      'sample_code': _clean(s['s_code']),
      'sample_rebeca': _clean(s['s_rebeca']),
      'sample_ccpi': _clean(s['s_ccpi']),
      'sample_date': _cleanDate(s['s_date']),
      'sample_country': _clean(s['s_country']),
      'sample_archipelago': _clean(s['s_archipelago']),
      'sample_island': _clean(s['s_island']),
      'sample_municipality': _clean(s['s_municipality']),
      'sample_local': _clean(s['s_local']),
      'sample_habitat_type': _clean(s['s_habitat_type']),
      'sample_habitat_1': _clean(s['s_habitat_1']),
      'sample_habitat_2': _clean(s['s_habitat_2']),
      'sample_habitat_3': _clean(s['s_habitat_3']),
      'sample_method': _clean(s['s_method']),
      'sample_photos': _clean(s['s_photos']),
      'sample_gps': _clean(s['s_gps']),
      'sample_temperature': _cleanDouble(s['s_temperature']),
      'sample_ph': _cleanDouble(s['s_ph']),
      'sample_conductivity': _cleanDouble(s['s_conductivity']),
      'sample_oxygen': _cleanDouble(s['s_oxygen']),
      'sample_salinity': _cleanDouble(s['s_salinity']),
      'sample_radiation': _cleanDouble(s['s_radiation']),
      'sample_responsible': _clean(s['s_responsible']),
      'sample_observations': _clean(s['s_observations']),
    }..removeWhere((k, v) => v == null);
  }
