import 'package:blue_open_lims/pages/culture_collection/function_printing_strains.dart';
...
class _StrainsPageState extends State<StrainsPage> {
          ...
          builder: (ctx, setDs) => AlertDialog(
                ...
                content: SizedBox(width: 360, child: Column(mainAxisSize: MainAxisSize.min, children: [
                  Text('A strain must originate from a sample.',
                      style: TextStyle(fontSize: 13, color: AppDS.textMuted)),
    ...
    return Scaffold(
      ...
      appBar: _selectionMode
          ...
          : buildStrainsNormalAppBar(
              ...
              onPrint: () => Navigator.push(context,
                  MaterialPageRoute(builder: (_) => const PrintStrainsPage())),
      ...
      body: Column(children: [
        ...
        if (_showFilters)
          StrainsFilterPanel(
            activeFilters: _activeFilters,
            kingdomMode: _kingdomMode,
            hideEmpty: _emptyColKeys.isNotEmpty,
            onDetectEmpty: () { _detectEmptyCols(); setState(() {}); },
            onShowEmpty: () => setState(() => _emptyColKeys = {}),
            onKingdomChanged: (v) => setState(() => _kingdomMode = v),
  ...
  Widget _buildGrid() {
    if (_filtered.isEmpty) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.science_outlined, size: 56, color: AppDS.textMuted),
        const SizedBox(height: 12),
        Text('No strains found', style: TextStyle(color: AppDS.textMuted, fontSize: 15)),
        const SizedBox(height: 16),
        FilledButton.icon(
            onPressed: _showAddStrainDialog,
            icon: const Icon(Icons.add),
            label: const Text('Add First Strain'),
            style: FilledButton.styleFrom(backgroundColor: AppDS.surface)),
      ]));
    }
      ...
      child: Row(children: [
        if (_selectionMode)
          SizedBox(width: AppDS.tableCheckW, child: Center(child: Checkbox(
            ...
            activeColor: Colors.white, checkColor: AppDS.surface,
            side: const BorderSide(color: Colors.white38, width: 1.5),
          ))),
        SizedBox(width: AppDS.tableOpenW,
            child: Center(child: Icon(Icons.launch_rounded, size: 13, color: Colors.white30))),
  ...
  Widget _buildHeaderCell(StrainColDef col, {required bool isColSelected}) {
    ...
    Color bg = blocked ? AppDS.border : Colors.transparent;
    if (isColSelected) bg = AppDS.blue800;
      ...
      child: Stack(clipBehavior: Clip.none, children: [
        MouseRegion(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
                color: bg,
                border: const Border(right: BorderSide(color: AppDS.border))),
            ...
            child: Row(children: [
              ...
              Expanded(child: Text(col.label,
                  style: AppDS.tableHeaderStyle.copyWith(
                    color: blocked ? Colors.white24 : isColSelected ? Colors.white
                        : col.readOnly ? Colors.white38 : AppDS.tableHeaderText),
                  overflow: TextOverflow.ellipsis)),
              if (!_selectionMode)
                if (blocked)
                  const Icon(Icons.block, size: 9, color: Colors.white24)
                else if (sortIndex >= 0)
  ...
  Widget _buildDataRow(Map<String, dynamic> row, int index, List<StrainColDef> cols,
    ...
    Color rowBg = isSelected ? AppDS.tableRowSel
        : highlight ? const Color(0xFFDEF1FF)
        : urgency == StrainTransferUrgency.overdue ? AppDS.tableRowUrgent
        : urgency == StrainTransferUrgency.soon    ? AppDS.tableRowSoon
        : index.isEven ? AppDS.tableRowEven : AppDS.tableRowOdd;
    final cellBase = isSelected ? AppDS.tableRowSel
        : index.isEven ? AppDS.tableRowEven : AppDS.tableRowOdd;
  ...
  Widget _buildDataCell(Map<String, dynamic> row, StrainColDef col, Color cellBase) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            color: blocked ? AppDS.tableRowUrgent : cellBase,
                ...
                child: isStatus && !blocked
                    ...
                    : Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        Flexible(child: Text(
                          ...
                          style: blocked
                              ? AppDS.tableCellStyle.copyWith(color: AppDS.red.withValues(alpha: 0.4))
                              : isReadOnly  ? AppDS.tableReadOnlyStyle
                              : isComputed  ? AppDS.tableCellStyle.copyWith(
                                    color: AppDS.blue500, fontStyle: FontStyle.italic)
                              : AppDS.tableCellStyle,
                          overflow: TextOverflow.ellipsis)),
                      ]),
              ),
      ),
