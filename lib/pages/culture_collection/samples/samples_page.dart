...
class _SamplesPageState extends State<SamplesPage> {
  ...
  PreferredSizeWidget _buildNormalAppBar(bool desktop) {
    Widget btn({required IconData icon, required String tooltip, required String label, required VoidCallback onPressed}) {
      if (desktop) {
        return Padding(
          ...
          child: TextButton.icon(
            icon: Icon(icon, size: 16, color: AppDS.toolbarIcon),
            label: Text(label, style: const TextStyle(fontSize: 12, color: AppDS.toolbarIcon)),
            onPressed: onPressed,
      ...
      return IconButton(
          icon: Icon(icon, size: 20, color: AppDS.toolbarIcon),
          tooltip: tooltip,
          onPressed: onPressed,
      ...
      actions: [
        ...
        if (desktop) ...[
          ...
          PopupMenuButton<String>(
            icon: const Icon(Icons.more_vert, color: AppDS.toolbarIcon),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(child: TextField(
            ...
            style: const TextStyle(fontSize: 13, color: AppDS.pageBg),
            decoration: InputDecoration(
              hintText: 'Search samples…',
              hintStyle: const TextStyle(color: AppDS.textMuted, fontSize: 13),
              prefixIcon: const Icon(Icons.search_rounded, color: AppDS.textMuted, size: 18),
              suffixIcon: _search.isNotEmpty
                  ? IconButton(icon: const Icon(Icons.clear, size: 16, color: AppDS.textMuted), onPressed: () {
          ...
          Container(
            ...
            child: Text('${_filtered.length} / ${_rows.length}',
                style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppDS.textMuted)),
          ),
        ]),
        if (hasSort) ...[
          const SizedBox(height: 8),
          SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
            const Text('Sort:', style: TextStyle(fontSize: 11, color: AppDS.textSecondary, fontWeight: FontWeight.w600)),
            const SizedBox(width: 6),
            ..._sortKeys.asMap().entries.map((e) => Padding(
              padding: const EdgeInsets.only(right: 4),
              child: InputChip(
                label: Text('${e.value} ${_sortDirs[e.value] == true ? "↑" : "↓"}',
                    style: const TextStyle(fontSize: 11, color: AppDS.pageBg)),
                selected: true,
            ...
            TextButton.icon(
              ...
              label: const Text('Clear sorts', style: TextStyle(fontSize: 12, color: AppDS.textSecondary)),
  ...
  Widget _buildColumnManager() {
    return Container(
      ...
      decoration: const BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppDS.shadow, blurRadius: 4, offset: Offset(0, 2))],
      ),
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            const Text('Show / Hide Columns',
                style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: AppDS.pageBg)),
            const Spacer(),
            TextButton(
          ...
          child: SingleChildScrollView(
            ...
            child: Wrap(
              ...
              children: sampleAllColumns.map((col) {
                ...
                return FilterChip(
                  label: Text(col.label, style: const TextStyle(fontSize: 11, color: AppDS.pageBg)),
                  ...
                  avatar: empty ? const Icon(Icons.remove_circle_outline, size: 11, color: AppDS.textMuted) : null,
  ...
  Widget _buildGrid() {
    if (_filtered.isEmpty) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Icon(Icons.colorize_outlined, size: 56, color: AppDS.border),
        const SizedBox(height: 12),
        const Text('No samples found', style: TextStyle(color: AppDS.textMuted, fontSize: 15)),
  ...
  Widget _buildHeaderRow(List<SampleColDef> cols) {
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
          color: AppDS.surface, border: Border(bottom: BorderSide(color: AppDS.border))),
      child: Row(children: [
        if (_selectionMode)
