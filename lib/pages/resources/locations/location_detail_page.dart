...
// ─── Design tokens (dark, matching app theme) ────────────────────────────────
class _DS {
...
class _LocationDetailPageState extends State<LocationDetailPage> {
  ...
  Widget build(BuildContext context) {
    final loc = _loc;
    final accent = loc != null ? LocationModel.typeAccent(loc.type) : _DS.accent;
      ...
      appBar: AppBar(
        ...
        title: Text(
          ...
          style: GoogleFonts.spaceGrotesk(
              color: _DS.textPrimary, fontWeight: FontWeight.w600),
        ),
        actions: [
          if (loc != null) ...[
            // QR copy
          ...
          : loc == null
              ? Center(
                  child: Text('Location not found',
                      style: GoogleFonts.spaceGrotesk(color: _DS.textMuted)))
              : _buildBody(loc, accent),
    );
  }

  Widget _buildBody(LocationModel loc, Color accent) {
    final qrData =
    ...
    return SingleChildScrollView(
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        // ── Header ────────────────────────────────────────────────────────────
        _buildHeader(loc, accent, qrData),
        ...
        Padding(
          ...
          child: Column(children: [
            _Section(
              ...
              child: _buildDetailsSection(loc),
            ),
            _Section(
              index: 1,
              title: 'NOTES',
  ...
  Widget _buildHeader(LocationModel loc, Color accent, String qrData) {
    return Container(
      color: _DS.surface2,
  ...
  Widget _buildDetailsSection(LocationModel loc) {
    final parentChoices =
      ...
      if (loc.createdAt != null) ...[
        ...
        Align(
          ...
          child: Text(
            ...
            style:
                GoogleFonts.spaceGrotesk(color: _DS.textMuted, fontSize: 11),
          ),
        ),
      ],
    ]);
  ...
  Widget _buildReagentsSection() {
    if (_reagents.isEmpty) {
      return Text('No reagents assigned to this location.',
          style:
              GoogleFonts.spaceGrotesk(color: _DS.textMuted, fontSize: 13));
    }
    return Column(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('QR — ${loc.name}',
            style: GoogleFonts.spaceGrotesk(color: _DS.textPrimary)),
        content: SizedBox(
          width: 260,
        ...
        actions: [
          TextButton(
            ...
            child: Text('Copy Link',
                style: GoogleFonts.spaceGrotesk(color: _DS.accent)),
          ),
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: Text('Close',
                style:
                    GoogleFonts.spaceGrotesk(color: _DS.textSecondary)),
          ),
        ],
      ),
    );
