...
String _maskedId(int index) =>
    _wellId(index ~/ _cols, index % _cols);
...
class _WellRandomizerPageState extends State<WellRandomizerPage> {
  ...
  void _applyExclusionMask(int layers) {
    setState(() {
      ...
      for (int r = 0; r < _rows; r++) {
        for (int c = 0; c < _cols; c++) {
          if (r < layers || r >= _rows - layers ||
              c < layers || c >= _cols - layers) {
            _excluded.add(_wellId(r, c));
          }
        }
      }
  ...
  void _randomize() {
    ...
    if (slots.length > _includedCount) {
      _toast('Total wells (${slots.length}) exceeds available '
             '($_includedCount). Reduce replicates or samples.', error: true);
      return;
    }
  ...
  Future<void> _exportCsv() async {
    try {
      ...
      final path = '${dir!.path}/well_plate_${DateTime.now().millisecondsSinceEpoch}.csv';
  ...
  Future<void> _exportImage() async {
    try {
      final boundary = _plateKey.currentContext!
          .findRenderObject() as RenderRepaintBoundary;
      final image = await boundary.toImage(pixelRatio: 3);
      final bytes = (await image.toByteData(format: ui.ImageByteFormat.png))!
          .buffer.asUint8List();
      final dir = await getDownloadsDirectory();
      final path = '${dir!.path}/well_plate_${DateTime.now().millisecondsSinceEpoch}.png';
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(children: [
          ...
          Text('96 Well Randomizer', style: GoogleFonts.spaceGrotesk(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text('Click any well on the plate to toggle.',
  ...
  Widget _totalsRow() {
    ...
    return Row(children: [
      Text('$_includedCount wells available · ',
        style: GoogleFonts.spaceGrotesk(
          fontSize: 11, color: context.appTextMuted)),
      Text('$total required',
    ...
    child: Row(children: [
      ...
      Expanded(
        child: Text(
          ...
          style: GoogleFonts.spaceGrotesk(
            fontSize: 11.5, color: const Color(0xFFF59E0B), height: 1.35),
        ),
      ),
    ]),
  );
  ...
  Widget _sampleCountRow() {
    ...
    return Row(children: [
      _roundIconBtn(Icons.remove_rounded,
      ...
      _roundIconBtn(Icons.add_rounded,
        () => _setUniqueCount(_uniqueCount + 1)),
    ...
    return Row(children: [
      Expanded(
        child: Column(
          ...
          children: [
            Text('Replicates (default)',
              style: GoogleFonts.spaceGrotesk(
                fontSize: 13, fontWeight: FontWeight.w600,
                color: context.appTextPrimary)),
            Text('Applied to all samples — override per-sample below',
              style: GoogleFonts.spaceGrotesk(
                fontSize: 11, color: context.appTextMuted)),
          ],
        ),
  ...
  Widget _maskToggleRow() => Row(children: [
    Switch(
      value: _masked,
  ...
  Widget _sectionLabel(String text) => Padding(
    ...
    child: Text(text.toUpperCase(),
      style: GoogleFonts.spaceGrotesk(
        fontSize: 11, fontWeight: FontWeight.w700,
        letterSpacing: 1.1, color: context.appTextMuted)),
  );
        ...
        children: [
          Row(children: [
            ...
            Container(
              ...
              child: Text('${_assignment.length}/$_totalReps assigned',
                style: GoogleFonts.jetBrainsMono(
                  fontSize: 11, fontWeight: FontWeight.w700,
                  color: _accent)),
            ),
            ...
            children: [
              SizedBox(
                ...
                child: Row(children: [
                  ...
                  ...List.generate(_cols, (c) => SizedBox(
              ...
              ...List.generate(_rows, (r) => SizedBox(
                height: cellSize,
                child: Row(children: [
                  SizedBox(
                    width: cellSize, height: cellSize,
            ...
            child: Center(
              child: excluded
                  ? Icon(Icons.close_rounded,
                      size: size * 0.42, color: context.appTextMuted)
                  : filled
                      ? Padding(
                          padding: EdgeInsets.all(size * 0.08),
                          child: FittedBox(
