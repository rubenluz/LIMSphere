...
class _BuilderTabState extends State<_BuilderTab> {
  ...
  void initState() {
    ...
    _checkConnection();
    _tpl =
        widget.template?.clone() ??
        LabelTemplate(id: 'new', name: 'New Template');
  ...
  Future<void> _loadZoom() async {
    ...
    if (!mounted) return;
    setState(() {
      _scale = (prefs.getDouble('label_builder_zoom') ?? 4.0).clamp(
        _minScale,
        12.0,
      );
    });
  ...
  double? get _printableW {
    ...
    if (p.protocol != 'brother_ql' && p.protocol != 'brother_ql_legacy')
      return null;
  ...
  double? get _printableH {
    ...
    if (p.protocol != 'brother_ql' && p.protocol != 'brother_ql_legacy')
      return null;
    return (_tpl.labelH - 3.0).clamp(1.0, _tpl.labelH);
  }
    ...
    setState(() {
      switch (axis) {
        ...
        case 'right':
          ...
          for (var i = 0; i < _tpl.fields.length; i++) {
            if (_selectedFieldIds.contains(_tpl.fields[i].id)) {
              _tpl.fields[i] = _tpl.fields[i].copyWith(
                x: maxR - _tpl.fields[i].w,
              );
            }
          }
        case 'centerH':
  ...
  Future<void> _togglePreview() async {
    ...
    try {
      ...
      final rows = await Supabase.instance.client
          .from(table)
          .select(_selectForCategory(_tpl.category))
          .limit(limit);
  ...
  List<Map<String, dynamic>> _sortPreviewRows(
    ...
    switch (category) {
      case 'Strains':
        rows.sort((a, b) {
          final na = (a['strain_name'] ?? a['strain_code'] ?? '')
              .toString()
              .toLowerCase();
          final nb = (b['strain_name'] ?? b['strain_code'] ?? '')
              .toString()
              .toLowerCase();
      ...
      case 'Samples':
        rows.sort(
          (a, b) => (a['sample_code'] ?? '').toString().compareTo(
            (b['sample_code'] ?? '').toString(),
          ),
        );
      case 'Reagents':
        rows.sort(
      ...
      case 'Equipment':
        rows.sort(
          (a, b) => (a['eq_name'] ?? '').toString().toLowerCase().compareTo(
            (b['eq_name'] ?? '').toString().toLowerCase(),
          ),
        );
      default:
        rows.sort(
          (a, b) => (a['name'] ?? a['code'] ?? '').toString().compareTo(
            (b['name'] ?? b['code'] ?? '').toString(),
          ),
        );
    }
    return rows;
  ...
  Future<void> _quickPrint() async {
    if (!context.requireModuleAction(ModuleAction.print)) return;
    if (_quickPrinting) return;
    final cfg = _activeProfile?.toPrinterConfig() ?? PrinterConfig();
    final tpl = _activeProfile?.applyTo(_tpl.clone()) ?? _tpl.clone();
    final row = _liveData != null ? [_liveData!] : <Map<String, dynamic>>[];
    setState(() => _quickPrinting = true);
    try {
      await _sendToPrinter(tpl, row, cfg);
      if (!mounted) return;
    ...
    if (mounted) setState(() => _quickPrinting = false);
  }

  void _addHourField() => _addField(
    LabelFieldType.text,
            ...
            children: [
              ...
              DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  value: _kPaperSizes.contains(_tpl.paperSize)
                      ? _tpl.paperSize
                      : _kPaperSizes.first,
                  isDense: true,
              ...
              Row(
                ...
                children: [
                  InkWell(
                    onTap: () {
                      setState(
                        () => _scale = (_scale - 0.5).clamp(_minScale, 12.0),
                      );
                      _saveZoom();
                    },
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: _livePreview ? AppDS.accent : context.appBorder,
                      ),
                    ),
              ...
              Tooltip(
                message: 'Quick print',
                child: InkWell(
                  onTap: _quickPrinting ? null : _quickPrint,
                  borderRadius: BorderRadius.circular(6),
                  child: Padding(
                    padding: const EdgeInsets.symmetric(
                      horizontal: 6,
              ...
              SizedBox(
                height: 20,
                child: VerticalDivider(color: context.appBorder),
              ),
              const SizedBox(width: 4),
              Text(
                'Copies',
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      Padding(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<int>(
                            ...
                            onChanged: (i) {
                              if (i != null) _setPreviewRow(i);
                            },
                          ),
                        ),
                      ),
                      Divider(height: 12, color: context.appBorder),
                    ],
                    Padding(
                    ...
                    Expanded(
                      child: ListView(
                        children: _tpl.fields
                            .map(
                              (f) => _FieldListItem(
                                field: f,
                                isSelected: _selectedFieldId == f.id,
                                isMultiSelected:
                                    _selectedFieldIds.contains(f.id) &&
                    ...
                    Expanded(
                      child: LayoutBuilder(
                        builder: (_, constraints) {
                          final fitW =
                              (constraints.maxWidth - 48) / _tpl.labelW;
                          final fitH =
                              (constraints.maxHeight - 48) / _tpl.labelH;
                          final newMin = (fitW < fitH ? fitW : fitH).clamp(
                            0.5,
                            4.0,
                          );
                          ...
                          return DragTarget<_FieldSpec>(
                            onAcceptWithDetails: (details) {
                              ...
                              final x = (local.dx / _scale).clamp(
                                0.0,
                                _tpl.labelW - 5.0,
                              );
                              final y = (local.dy / _scale).clamp(
                                0.0,
                                _tpl.labelH - 3.0,
                              );
                              _addField(
                            ...
                            builder: (context, candidateData, _) => Listener(
                              onPointerSignal: (event) {
                                if (event is PointerScrollEvent &&
                                  ...
                                  setState(
                                    () => _scale =
                                        (_scale - event.scrollDelta.dy * 0.008)
                                            .clamp(_minScale, 12.0),
                                  );
                                  _saveZoom();
                                }
                              },
                                        ...
                                        child: _BuilderCanvas(
                                          ...
                                          onMove: (id, dx, dy) {
                                            ...
                                            if (f != null) {
                                              _updateField(
                                                f.copyWith(
                                                  x: (f.x + dx / _scale).clamp(
                                                    0,
                                                    _tpl.labelW - 5,
                                                  ),
                                                  y: (f.y + dy / _scale).clamp(
                                                    0,
                                                    _tpl.labelH - 3,
                                                  ),
                                                ),
                                          ...
                                          onResize: (id, dw, dh) {
                                            ...
                                            if (f != null) {
                                              var newW = (f.w + dw / _scale)
                                                  .clamp(10.0, _tpl.labelW);
                                              var newH = (f.h + dh / _scale)
                                                  .clamp(5.0, _tpl.labelH);
                                              if (f.type ==
                                                  LabelFieldType.qrcode) {
                                                final s =
                                                    math
                                                            .max(newW, newH)
                                                            .clamp(
                    ...
                    child: Column(
                      ...
                      children: [
                        if (_selectedFieldIds.length > 1) ...[
                          Text(
                            'ALIGN ${_selectedFieldIds.length} FIELDS',
                            style: TextStyle(
                              fontSize: 9,
                              letterSpacing: 1.2,
                              color: context.appTextSecondary,
