...
Future<void> showQuickPrintDialog(
  ...
  if (!context.requireModuleAction(ModuleAction.print)) {
    return Future<void>.value();
  }
  final access = context.moduleAccess;
  return showDialog<void>(
    context: context,
    builder: (_) => ModulePermission(
      access: access,
      child: _QuickPrintDialog(
        category: category,
        data: data,
        entityId: entityId,
      ),
  ...
  Future<void> _load() async {
    try {
      ...
      final templates = rows
          .map((r) {
            try {
              return LabelTemplate.fromDb(r as Map<String, dynamic>);
            } catch (_) {
              return null;
            }
          })
