...
class _PrintLabelPageState extends State<_PrintLabelPage> {
  ...
  Future<void> _loadPrefs() async {
    ...
    setState(() {
      ...
      _statusFilter =
          prefs.getString('print_status_${widget.entityType}') ?? '';
      _searchCtrl.text = _search;
      final filtersRaw =
          prefs.getStringList('print_filters_${widget.entityType}') ?? [];
      _activeFilters
        ..clear()
        ..addAll(
          filtersRaw.map(_ActiveFilter.fromEncoded).whereType<_ActiveFilter>(),
        );
      final sortsRaw =
          prefs.getStringList('print_sorts_${widget.entityType}') ?? [];
      _activeSorts
        ..clear()
        ..addAll(
          sortsRaw.map(_SortConfig.fromEncoded).whereType<_SortConfig>(),
        );
    });
  }
  ...
  Future<void> _savePrefs() async {
    ...
    await prefs.setStringList(
      'print_filters_${widget.entityType}',
      _activeFilters.map((f) => f.encoded).toList(),
    );
    await prefs.setStringList(
      'print_sorts_${widget.entityType}',
    ...
    try {
      final rows =
          await Supabase.instance.client
                  ...
                  .select(_selectForCategory(widget.entityType))
              as List<dynamic>;
  ...
  Future<void> _doPrint() async {
    ...
    final cfg = _activeProfile?.toPrinterConfig() ?? PrinterConfig();
    final proto =
        (cfg.protocol == 'brother_ql' || cfg.protocol == 'brother_ql_legacy')
        ? 'Brother QL'
        : 'ZPL';
    setState(() {
      _isPrinting = true;
      _status = 'Generating $proto data…';
    });
    try {
      final batch = _selectedRecords.isEmpty
          ? <Map<String, dynamic>>[]
          : _selectedRecords;
                ...
                : Column(
                    children: [
                      _FilterBar(
                        ...
                        onToggleSortDir: (i) {
                          setState(
                            () => _activeSorts[i] = _activeSorts[i].toggled(),
                          );
                          _savePrefs();
                        },
                      ),
