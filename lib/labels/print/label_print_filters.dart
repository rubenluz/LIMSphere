        ...
        children: [
          ...
          Row(
            children: [
              ...
              if (widget.filterLabel.isNotEmpty &&
                  widget.filterOptions.isNotEmpty) ...[
                const SizedBox(width: 8),
                DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    value: widget.statusFilter.isEmpty
                        ? null
                        : widget.statusFilter,
                    hint: Text(
                      widget.filterLabel,
              ...
              Wrap(
                ...
                children: [
                  for (var i = 0; i < widget.activeFilters.length; i++)
                    _FilterChip(
                      filter: widget.activeFilters[i],
                      onRemove: () => widget.onRemoveFilter(i),
                    ),
                ],
              ),
            ],
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: Autocomplete<String>(
                    ...
                    optionsBuilder: (v) {
                      final q = v.text.toLowerCase();
                      final used = widget.activeSorts
                          .map((s) => s.field)
                          .toSet();
                      final available = widget.allFields.where(
                        (c) => !used.contains(c),
                      );
                      return q.isEmpty
                          ? available
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Text(
            '${_colLabel(filter.field)} $_modeLabel ${filter.value}',
