...
class _LocationDetailPageState extends State<LocationDetailPage> {
  ...
  Future<void> _save() async {
    ...
    try {
      final data = <String, dynamic>{
        ...
        'location_type': _type,
        'location_temperature': _tempCtrl.text.trim().isEmpty
            ? null
            : _tempCtrl.text.trim(),
  ...
  Widget build(BuildContext context) {
    final loc = _loc;
    final accent = loc != null
        ? LocationModel.typeAccent(loc.type)
        : AppDS.accent;
  ...
  Widget _buildDetailsSection(BuildContext context, LocationModel loc) {
    ...
    final selectedParentId = parentChoices.any((room) => room.id == _parentId)
        ? _parentId
        : null;
          ...
          children: [
            ...
            DetailInlineDropdown<String>(
              ...
              onChanged: (v) => setState(
                () => _type = v ?? LocationModel.defaultLocationType,
              ),
            ),
          ],
        ),
              ...
              items: [
                ...
                ...parentChoices.map(
                  (l) => DropdownMenuItem<int?>(
                    value: l.id,
                    child: Text(
                      stripLocationCodePrefix(l.name),
                      style: GoogleFonts.spaceGrotesk(
                        color: context.appTextPrimary,
                        fontSize: 13,
                      ),
  ...
  Widget _buildReagentsSection(BuildContext context) {
    if (_reagents.isEmpty) {
      return Text(
        'No reagents assigned to this location.',
        style: GoogleFonts.spaceGrotesk(
          color: context.appTextMuted,
          fontSize: 13,
        ),
            ...
            children: [
              ...
              if (expiry != null) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (expired ? AppDS.red : AppDS.yellow).withValues(
                      alpha: 0.15,
                    ),
